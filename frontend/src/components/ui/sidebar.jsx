...
const SidebarProvider = React.forwardRef(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
    ...
    // Đây là trạng thái đóng mở của sidebar.
    ...
    const setOpen = React.useCallback((value) => {
        ...
        // Lưu lại trạng thái sidebar trong cookie.
        document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    }, [setOpenProp, open]);
    // Hàm này dùng để đóng hoặc mở sidebar.
    const toggleSidebar = React.useCallback(() => {
        return isMobile ? setOpenMobile((open) => !open) : setOpen((open) => !open);
    }, [isMobile, setOpen, setOpenMobile]);
    // Chỗ này thêm phím tắt để đóng mở sidebar.
    React.useEffect(() => {
    ...
    // Đổi trạng thái thành expanded hoặc collapsed để gắn class cho dễ.
...
const Sidebar = React.forwardRef(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
    ...
    return (<div ref={ref} className="group peer hidden text-sidebar-foreground md:block" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side}>
        {/* Đây là khoảng trống của sidebar trên màn hình lớn */}
        ...
        // Chỉnh khoảng cách cho kiểu floating với inset.
...
const SidebarGroupAction = React.forwardRef(({ className, asChild = false, ...props }, ref) => {
    ...
    return (<Comp ref={ref} data-sidebar="group-action" className={cn("absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring cursor-pointer transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0", 
        // Cho vùng bấm của nút rộng hơn trên điện thoại.
